...
import IpaVoiceLab from './components/IpaVoiceLab';
...
const profiles = {
  tongyu: { name: '宋代文人通語', subtitle: '默认基线 · 北宋桥接', note: '以《廣韻》音韻地位和北宋推導為可審查基線；宋人音注與宋詞實際用韻尚待逐字校驗。' },
  linan: { name: '南宋臨安實現', subtitle: '实验方案 · 1127–1279', note: '在宋代文人通語之上研究行都接觸層；目前音值仍沿用通語基線，不假造未證實的地方差異。' },
} as const;
...
export default function Home() {
  ...
  const [profile, setProfile] = useState<ProfileId>('tongyu');
  const [selected, setSelected] = useState(0);
  const [rate, setRate] = useState(0.86);
  const [pause, setPause] = useState(270);
  const [mode, setMode] = useState<'study' | 'ipa' | 'neural' | 'system'>('study');
  ...
  async function downloadResearchAudio() {
    ...
    anchor.href = href;
    anchor.download = `song-ci-${profile}-research-voice.wav`;
    anchor.click();
    window.setTimeout(() => URL.revokeObjectURL(href), 500);
  }
  return <main className="site-shell">
    <header className="topbar">
      <a className="brand" href="#top" aria-label="宋词声景首页"><span>宋</span><b>宋词声景</b></a>
      <nav className="page-nav" aria-label="页面目录"><a href="#input">输入</a><a href="#rhyme">韵脚</a><a href="#phonology">拟音</a><a href="#listening">试听</a></nav>
      <div className="topbar-note">AI vibe 原型 · 未经验证</div>
      <button className="text-button" onClick={downloadAnnotation}>导出注释</button>
    </header>
    <div className="global-disclaimer"><UnverifiedNotice area="本网站所有入口与导出均应按此状态理解。" /></div>

    <section className="hero" id="top">
      <div><p className="eyebrow">SONG LYRIC READING · UNVALIDATED</p><h1>让宋词以一套<br /><em>可追溯、可修订</em>的声音被听见。</h1><p className="hero-copy">先重建跨地域的宋代文人通语，再讨论南宋临安的城市实现；不把任何现代方言直接当作宋词原音。</p></div>
      <aside className="principle-card"><span className="card-kicker">当前未验证方案</span><strong>{profiles[profile].name}</strong><p>{profiles[profile].note}</p><div><span className="dot" />证据等级是资料标记，不是验证结论</div></aside>
    </section>

    <section className="workspace" id="input" aria-label="宋词拟音工作台">
      <div className="controls-column">
        <div className="section-heading"><span>01</span><h2>选择历史层</h2></div>
        <div className="profile-switch" role="radiogroup" aria-label="选择拟音方案">{(Object.keys(profiles) as ProfileId[]).map((id) => <button key={id} className={profile === id ? 'profile active' : 'profile'} onClick={() => setProfile(id)} role="radio" aria-checked={profile === id}><b>{profiles[id].name}</b><small>{profiles[id].subtitle}</small></button>)}</div>
        <div className="method-note"><b>方案关系</b><p>“宋代文人通语”是默认基线；“南宋临安实现”是它的地域化研究层。当前两者尚使用同一 IPA，页面只改变证据范围，不伪造音值差异。</p><p className="secondary-source-note">婺州／金华资料降为南部吴语比较材料，不再作为主声景。</p><UnverifiedNotice area="时代与地域选择" className="inline-notice" /></div>
      </div>
      <div className="input-column">
        <div className="section-heading"><span>02</span><h2>粘贴宋词或古文</h2><small>{text.length} 字符</small></div>
        <textarea value={text} onChange={(event) => { setText(event.target.value); setOverrides({}); }} aria-label="宋词或古文文本输入" placeholder="在这里粘贴宋词、诗作或古文…" />
        <div className="input-footer"><button className="sample-button" onClick={() => { setText(sampleText); setOverrides({}); }}>载入《水调歌头》节选</button><span>{analysisText === text ? '《廣韻》可查字以规则推导；未录字标为待考' : '已在本地转为繁体字形后查《廣韻》；原文保持不变'}</span></div>
        <UnverifiedNotice area="自动查字、断句与义读" className="inline-notice" />
      </div>
    </section>
    ...
    <section className="analysis-section" id="phonology">
      <div className="analysis-header"><div><p className="eyebrow">PHONOLOGICAL TRACE</p><h2>再审逐字拟音</h2><UnverifiedNotice area="韵书地位到宋代通语／临安 IPA 的转换" className="inline-notice" /></div><div className="coverage"><b>{covered}<small> / {readings.length}</small></b><span>已具可审查读音（非验证）</span></div></div>
      <div className="token-grid">{readings.length === 0 ? <p className="empty-state">请输入至少一个汉字以开始分析。</p> : readings.map((reading, index) => <button className={selectedIndex === index ? `token confidence-${reading.confidence} selected` : `token confidence-${reading.confidence}`} onClick={() => setSelected(index)} key={`${reading.character}-${index}`}><b>{reading.character}</b><span>{reading.ipa}</span><i>{reading.confidence}</i></button>)}</div>
      {active && <article className="evidence-panel"><div className="character-mark">{active.character}</div><div><div><span className={`confidence-badge confidence-${active.categoryConfidence}`}>{active.categoryConfidence} · 韻書地位</span> <span className={`confidence-badge confidence-${active.periodConfidence}`}>{active.periodConfidence} · 宋代橋接</span> <span className={`confidence-badge confidence-${active.regionalConfidence}`}>{active.regionalConfidence} · {profile === 'tongyu' ? '通語基線' : '臨安實現'}</span></div><h3>{active.position}</h3><p><b>释义：</b>{active.meaning}　<b>拟音：</b><code>{active.ipa}</code>　<b>白一平转写：</b><code>{active.baxter}</code></p><p className="evidence"><b>依据：</b>{active.evidence}</p>{showAlternatives && <div className="alternative-list">{alternatives.map((candidate, index) => <button key={`${candidate.position}-${index}`} className={candidate.position === active.position ? 'chosen' : ''} onClick={() => { setOverrides((current) => ({ ...current, [selectedIndex]: index })); setShowAlternatives(false); }}><b>{candidate.position}</b><code>{candidate.ipa}</code><span>{candidate.meaning}</span></button>)}</div>}</div><button className="change-reading" onClick={() => setShowAlternatives((open) => !open)}>{showAlternatives ? '收起候选' : `选择替代读法${alternatives.length > 1 ? `（${alternatives.length}）` : ''}`}</button></article>}
    </section>
    ...
    <section className="listen-section" id="listening">
      <div>
        ...
        <p className="listen-copy">为什么仍不像熟悉的吴语？当前不是现代吴语复刻：研究合成尚未实现清浊相关的阴阳调域、系统的发声态差异或词组连读变调；音素女声也未控制这些特征。听起来更像中文，不等于宋音更准确。</p>
        <p className="timing-note">平声高平 · 上声先抑后扬 · 去声高起陡降 · 入声短而闭锁。每一枚标点静音一个普通舒声音节：当前约 {Math.round(punctuationPauseMs(rate))} ms。</p>
        <UnverifiedNotice area="研究合成、音素女声实验、神经代理与设备声线" className="inline-notice" />
      </div>
      <div className="voice-panel"><div className="mode-tabs"><button onClick={() => setMode('study')} className={mode === 'study' ? 'active' : ''}>研究合成 <small>IPA 可控 · v7</small></button><button onClick={() => { stop(); setMode('ipa'); }} className={mode === 'ipa' ? 'active' : ''}>音素女声 <small>免训练 · 实验</small></button><button onClick={() => setMode('neural')} className={mode === 'neural' ? 'active' : ''}>神经代理 <small>现代普通话</small></button><button onClick={() => setMode('system')} className={mode === 'system' ? 'active' : ''}>设备声线 <small>系统预听</small></button></div>{mode === 'ipa' && <IpaVoiceLab key={JSON.stringify([analysisText, readings, rate])} readings={readings} rate={rate} gapsMs={readings.map((_, i) => gapFor(i))} initialGapMs={initialGapMs} onStart={stop} onLoadSample={() => { stop(); setText('明月，清風。'); setOverrides({}); }} />}{mode === 'neural' && <div className="neural-note"><b>Kokoro 中文女声 · 在线模型</b><p>首次使用需下载并缓存上百 MB；之后在浏览器本地推理。它不读取本页 IPA，也不能验证宋代文人通语或临安实现。</p><span>{neuralStatus}</span></div>}{mode === 'system' && <label className="voice-picker">选择设备声线 <select value={voiceUri} onChange={(event) => setVoiceUri(event.target.value)} disabled={!voices.length}>{voices.length ? voices.map((voice) => <option value={voice.voiceURI} key={voice.voiceURI}>{voice.name} · {voice.lang}</option>) : <option>未检测到中文声线</option>}</select><small>请在设备提供的中文声线中选择女性声线。</small></label>}{mode !== 'ipa' && <><div className={mode === 'study' ? 'sliders' : 'sliders single'}><label>语速 <output>{rate.toFixed(2)}×</output><input type="range" min="0.65" max="1.15" step="0.01" value={rate} onChange={(event) => setRate(Number(event.target.value))} /></label>{mode === 'study' && <label>句间停顿 <output>{pause}ms</output><input type="range" min="120" max="720" step="30" value={pause} onChange={(event) => setPause(Number(event.target.value))} /></label>}</div>{mode === 'study' && <label className="rhyme-toggle"><input type="checkbox" checked={emphasizeRhyme} onChange={(event) => setEmphasizeRhyme(event.target.checked)} />强化句末韵脚时值 <small>入声只轻微延长，仍保持短促</small></label>}<div className="player-row"><button className={isPlaying ? 'stop-button' : 'play-button'} onClick={isPlaying ? stop : mode === 'study' ? playStudyVoice : mode === 'neural' ? playNeuralVoice : playSystemVoice}>{isPlaying ? '■ 停止' : mode === 'neural' ? '▶ 加载并朗读' : '▶ 开始朗读'}</button>{mode === 'study' && <button className="export-audio" onClick={downloadResearchAudio}>下载 48 kHz WAV</button>}<span>{mode === 'study' ? '舒声约 630 ms · 入声约 390 ms' : mode === 'neural' ? '自然度比较 · 非历史拟音' : '取决于设备声线'}</span></div></>}</div>
    </section>
