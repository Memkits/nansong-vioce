...
export default function Home() {
  ...
  function downloadAnnotation() {
    const payload = { system: '南宋吴地拟音古文朗读系统', profile: profiles[profile], dataVersion: '0.6.0-experimental', createdAt: new Date().toISOString(), sourceText: text, analysisText, readings, sentenceData, dominantRhyme, emphasizeRhyme, audio: { sampleRate: RESEARCH_SAMPLE_RATE, renderer: 'deterministic-articulated-v4', neuralProxy: neuralModel } };
  ...
  return <main className="site-shell">
    ...
    <section className="listen-section"><div><p className="eyebrow">LISTENING ROOM</p><h2>聆听这个方案</h2><p className="listen-copy">研究合成 v4 继续由 IPA 控制：削减元音段底噪，缩短并强化塞音／塞擦音爆破，拉开辅音与元音边界；神经代理用来比较真人感，但只读现代普通话。</p><UnverifiedNotice area="研究合成、神经代理与设备声线" className="inline-notice" /></div><div className="voice-panel"><div className="mode-tabs"><button onClick={() => setMode('study')} className={mode === 'study' ? 'active' : ''}>研究合成 <small>IPA 可控 · v4</small></button><button onClick={() => setMode('neural')} className={mode === 'neural' ? 'active' : ''}>神经代理 <small>现代普通话</small></button><button onClick={() => setMode('system')} className={mode === 'system' ? 'active' : ''}>设备声线 <small>系统预听</small></button></div>{mode === 'neural' && <div className="neural-note"><b>Kokoro 中文女声 · 在线模型</b><p>首次使用需下载并缓存上百 MB；之后在浏览器本地推理。它不读取本页 IPA，也不能验证临安／金华发音。</p><span>{neuralStatus}</span></div>}{mode === 'system' && <label className="voice-picker">选择设备声线 <select value={voiceUri} onChange={(event) => setVoiceUri(event.target.value)} disabled={!voices.length}>{voices.length ? voices.map((voice) => <option value={voice.voiceURI} key={voice.voiceURI}>{voice.name} · {voice.lang}</option>) : <option>未检测到中文声线</option>}</select><small>请在设备提供的中文声线中选择女性声线。</small></label>}<div className={mode === 'study' ? 'sliders' : 'sliders single'}><label>语速 <output>{rate.toFixed(2)}×</output><input type="range" min="0.65" max="1.15" step="0.01" value={rate} onChange={(event) => setRate(Number(event.target.value))} /></label>{mode === 'study' && <label>句间停顿 <output>{pause}ms</output><input type="range" min="120" max="720" step="30" value={pause} onChange={(event) => setPause(Number(event.target.value))} /></label>}</div>{mode === 'study' && <label className="rhyme-toggle"><input type="checkbox" checked={emphasizeRhyme} onChange={(event) => setEmphasizeRhyme(event.target.checked)} />强化句末韵脚时值 <small>仅影响研究合成与 WAV</small></label>}<div className="player-row"><button className={isPlaying ? 'stop-button' : 'play-button'} onClick={isPlaying ? stop : mode === 'study' ? playStudyVoice : mode === 'neural' ? playNeuralVoice : playSystemVoice}>{isPlaying ? '■ 停止' : mode === 'neural' ? '▶ 加载并朗读' : '▶ 开始朗读'}</button>{mode === 'study' && <button className="export-audio" onClick={downloadResearchAudio}>下载 48 kHz WAV</button>}<span>{mode === 'study' ? '低底噪 DSP · 未经听辨验证' : mode === 'neural' ? '自然度比较 · 非历史拟音' : '取决于设备声线'}</span></div></div></section>
